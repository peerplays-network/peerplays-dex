	...
	return (
		<Card mode="widget" style={{ height:"100%"}}>
			<div className="card__title" style={{ paddingTop:"20px" , borderTopLeftRadius:"10px" , borderTopRightRadius:"10px"}}>
				{counterpart.translate(`voting.powerUp`)}
			</div>
			<CardContent >
				...
				<span style={{ fontWeight:"bold",margin:"10px",display:"block"}}>
					{counterpart.translate(`deposit.title`)}
				</span>
				...
				<div style={{ marginTop: 12, color: "#ff444a", display: (changes &&(vestAmount == null || vestAmount == 0)) ? "block" : "none" }}>
					<div>{counterpart.translate(`errors.requiredAndnotzero`)}</div>
				</div>
				<div style={{ marginTop: 12, color: "#ff444a", display: (vestAmount == null || vestAmount > accBalance) ? "block" : "none" }}>
					<div>{counterpart.translate("errors.requiredAndnotzero"+ accBalance)}</div>
				</div>
				<div style={{ marginTop: 12 }} className="input-cus-style">
					<div style={{padding:"0 10px"}}>
						<span>{counterpart.translate(`voting.newGpos`)}</span>: <strong style={{padding:"0 10px"}}>{totalGpos + vestAmount} {symbol}</strong>
					</div>
				</div>
			</CardContent>
			<div className="info__row margin">
				<span>
					<span>{counterpart.translate(`field.labels.fee`)}</span> {fee} {getBasicAsset().symbol}
				</span>
			{error &&
			...
			{sended &&
				<span className="clr--positive">{counterpart.translate(`voting.trans`)}</span>
			}
		  </div>
			<CardActions style={{justifyContent:"end"}} >
				<button 
					className="btn-round btn-round--buy" 
					onClick={() => (vestAmount == null || vestAmount == 0 || vestAmount > accBalance) ? setChanges(true) : SubmitGposVesting()}
				>
					{counterpart.translate(`voting.Vest`)}
				</button>
			</CardActions>
		</Card>
	)
